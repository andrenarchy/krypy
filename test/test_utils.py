...
def test_projection(X, Ys, ip_B, iterations):
    ...
    if k > 0:
        ...
        assert_almost_equal(
            numpy.linalg.norm(
                krypy.utils.inner(X if Y is None else Y, I - P.apply(I), ip_B=ip_B), 2
            ),
            0,
            13,
        )
    ...
    assert_array_almost_equal(
        Ya, krypy.utils.inner(X if Y is None else Y, a, ip_B=ip_B)
    )
...
def test_qr(X, ip_B, reorthos):
    ...
    assert (
        numpy.linalg.norm(krypy.utils.inner(Q, Q, ip_B=ip_B) - numpy.eye(k), 2)
        <= orthotol
    )
...
def test_angles(F, G, ip_B, compute_vectors):
    if compute_vectors:
        ...
        theta = krypy.utils.angles(F, G, ip_B=ip_B, compute_vectors=compute_vectors)
...
def test_hegedus(matrix, get_operator, x, x0, M, Ml, ip_B):
    ...
    MMlr0new_norm = krypy.utils.norm(Mlr0new, M * Mlr0new, ip_B=ip_B)
...
def assert_arnoldi(
    ...
    proj_tol = proj_const * (
        ortho_resn * An + arnoldi_resn * krypy.utils.norm(V, ip_B=ip_B)
    )
...
def test_ritz(matrix, get_operator, v, maxiter, ip_B, with_V, type):
    ...
    if type == "ritz":
        assert (
            numpy.linalg.norm(krypy.utils.inner(V[:, :n], R, ip_B=ip_B), 2)
            <= 1e-14 * An
        )
    elif type == "harmonic":
        AVortho = scipy.linalg.orth(A * V[:, :n])
        assert (
            numpy.linalg.norm(krypy.utils.inner(AVortho, R, ip_B=ip_B), 2) <= 1e-12 * An
        )
    else:
        pass
...
def test_BoundCG():
    ...
    b = krypy.utils.BoundCG(
        krypy.utils.Intervals([krypy.utils.Interval(1, 1.2), krypy.utils.Interval(2)])
    )
    assert_almost_equal(b.eval_step(8), 1.5018239652065932e-06)
    assert ceil(b.get_step(1e-6)) == 9
...
def test_BoundMinres():
    ...
    b = krypy.utils.BoundMinres(
        krypy.utils.Intervals([krypy.utils.Interval(-2, -1), krypy.utils.Interval(2)])
    )
    assert_almost_equal(b.eval_step(8), 0.0017331035544401801)
    assert ceil(b.get_step(2e-3)) == 8
